...
public class BoardController {
	...
	@PostMapping("/remove")
	public String remove(Long bno, RedirectAttributes rttr, SearchCriteria cri, Model model) {

		
		// 먼저 첨부파일 정보를 attachList 변수에 저장하기
		List<BoardAttachVO> attachList = service.getAttachList(bno);
		
		
		// 글 삭제 로직이 정상적으로 처리된 경우에만
		if (service.remove(bno)) {
			// 첨부파일들을 실제 폴더에서 마저 삭제합니다.
	...
	private void deleteFiles(List<BoardAttachVO> attachList) {
		...
		log.info(attachList);
		// 넘겨받은 첨부파일들을 하나하나 삭제하는 로직
		attachList.forEach(attach -> {
			try {
				Path file = Paths.get("c:\\upload_data\\temp\\" + attach.getUploadPath() + "\\" + attach.getUuid() + "_"
						+ attach.getFileName());

				Files.deleteIfExists(file);
				// 그림파일인 경우, 썸네일까지 추가로 삭제
				// 파일타입이 이미지인 경우부터 체크하도록 처리
				if (Files.probeContentType(file).startsWith("image")) {
					Path thumbNail = Paths.get("C:\\upload_data\\temp\\" + attach.getUploadPath() + "\\s_"
